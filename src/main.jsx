...
function Shop({ mode }) {
  ...
  const isEmpty = ['top-brands', 'nearby-stores', 'home', 'explore', 'profile'].includes(mode);
  const emptyTitle = mode === 'top-brands' ? 'Top Brands' : mode === 'nearby-stores' ? 'Nearby Stores' : mode === 'explore' ? 'Explore' : mode === 'profile' ? 'Profile' : 'Home';
  if (isEmpty) return <main className="shop-shell"><header className="shop-header"><a className="shop-logo" href="/shop"><span>1</span>1Fi</a><p>Shop</p></header><section className="shop-empty"><button className="back-link" onClick={() => go('/shop')}>&lt; Shop</button><div className="empty-icon">{mode === 'top-brands' ? 'TB' : mode === 'nearby-stores' ? 'NS' : mode === 'explore' ? 'E' : mode === 'profile' ? 'P' : 'H'}</div><h1>{emptyTitle}</h1><p>{mode === 'top-brands' || mode === 'nearby-stores' ? 'This section is intentionally kept blank for the assignment.' : 'This area is ready for future 1Fi app content.'}</p></section><nav className="bottom-nav"><button className={mode === 'home' ? 'active' : ''} onClick={() => go('/home')}>Home</button><button className={mode === 'explore' ? 'active' : ''} onClick={() => go('/explore')}>Explore</button><button onClick={() => go('/shop')}>Shop</button><button onClick={() => go('/profile')}>Profile</button></nav></main>;
  return <main className="shop-shell"><header className="shop-header"><a className="shop-logo" href="/shop"><span>1</span>1Fi</a><p>Shop</p></header><section className="shop-hero"><p>SHOP WITH 1FI</p><h1>Find more ways to make your money work for you.</h1><span>Curated rewards, local offers, and flexible ways to own what you love.</span></section><section className="shop-options"><p className="section-label">EXPLORE SHOP</p><button className="shop-option" onClick={() => go('/shop/top-brands')}><i className="option-icon brands">T</i><span><strong>Top Brands</strong><small>Offers from brands you love</small></span><Icon name="chevron" /></button><button className="shop-option" onClick={() => go('/shop/nearby-stores')}><i className="option-icon nearby">N</i><span><strong>Nearby Stores</strong><small>Discover offers around you</small></span><Icon name="chevron" /></button><button className="shop-option marketplace-option" onClick={() => go('/shop/marketplace')}><i className="option-icon market">1</i><span><strong>1Fi Marketplace</strong><small>Own more. Invest as you pay.</small></span><Icon name="chevron" /></button></section><nav className="bottom-nav"><button onClick={() => go('/home')}>Home</button><button onClick={() => go('/explore')}>Explore</button><button className="active" onClick={() => go('/shop')}>Shop</button><button onClick={() => go('/profile')}>Profile</button></nav></main>;
}
...
function App() {
  ...
  const shopMode = location.pathname === '/home' ? 'home' : location.pathname === '/explore' ? 'explore' : location.pathname === '/profile' ? 'profile' : location.pathname.match(/^\/shop(?:\/(top-brands|nearby-stores))?$/)?.[1] || (location.pathname === '/shop' || location.pathname === '/' ? 'shop' : null);
